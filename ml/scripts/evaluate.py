...
def main():
    ...
    # Req-Qual-Perf-1: convert MSE to a 0-1 accuracy scale.
    # We normalize each metric's MSE by the variance of that metric in the
    # test set: MSE == variance → accuracy = 0 (model no better than mean),
    # MSE == 0 → accuracy = 1 (perfect).
    def _variance(arr) -> float:
        v = float(torch.tensor(arr).float().var().item())
        return v if v > 0 else 1.0

    var_lat = _variance(y_test[:, :, 0])
    var_jit = _variance(y_test[:, :, 1])
    var_pkt = _variance(y_test[:, :, 2])

    acc_lat = max(0.0, 1 - results["mse_latency"] / var_lat)
    acc_jit = max(0.0, 1 - results["mse_jitter"] / var_jit)
    acc_pkt = max(0.0, 1 - results["mse_packet_loss"] / var_pkt)
    accuracy_overall = (acc_lat + acc_jit + acc_pkt) / 3
    ...
    report = {
        ...
        "overall_accuracy": round(accuracy_overall, 4),
        "meets_pvd_target": accuracy_overall >= 0.90,
        "pvd_target": 0.90,
        "per_metric_accuracy": {
            "latency": round(acc_lat, 4),
            "jitter": round(acc_jit, 4),
            "packet_loss": round(acc_pkt, 4),
        },
        "mse": {
            "latency": round(results["mse_latency"], 6),
            "jitter": round(results["mse_jitter"], 6),
            "packet_loss": round(results["mse_packet_loss"], 6),
        },
        "mae": {
            "latency": round(results.get("mae_latency", 0.0), 6),
            "jitter": round(results.get("mae_jitter", 0.0), 6),
            "packet_loss": round(results.get("mae_packet_loss", 0.0), 6),
        },
        "brownout_recall": round(results.get("brownout_recall", 0.0), 4),
        "false_positive_rate": round(results.get("false_positive_rate", 0.0), 4),
        "checkpoint": str(checkpoint_path),
    }

    out_path = Path("ml/checkpoints/accuracy.json")
    ...
    logger.info("=" * 60)
    logger.info(f"  Overall accuracy: {accuracy_overall:.2%} (PVD target ≥ 90%)")
    logger.info(f"  Meets PVD target: {report['meets_pvd_target']}")
    for key, value in results.items():
        logger.info(f"  {key}: {value:.6f}")
    logger.info(f"  Wrote report to {out_path}")
    logger.info("=" * 60)

    # Fail hard so CI catches accuracy regressions (skip for smoke tests)
    if not args.smoke_test and not report["meets_pvd_target"]:
        logger.error("LSTM accuracy below PVD target of 90% — failing CI gate")
        sys.exit(1)
