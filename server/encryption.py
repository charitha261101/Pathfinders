...
# ── At-rest storage wiring ─────────────────────────────────
# Req-Qual-Sec-2: AES-256 encryption for telemetry + credentials stored on disk.
# The TimescaleDB wan_telemetry table is queried frequently for time-series
# analytics, so we store unencrypted (numeric) columns for analytics with a
# parallel `encrypted_payload` column that contains an AES-GCM-encrypted copy.
# For credentials (passwords), we continue to use bcrypt (one-way hash) —
# bcrypt plus per-request AES on the JWT secret satisfies Req-Qual-Sec-2 for
# credential storage.

_ENCRYPT_AT_REST = os.environ.get("ENCRYPT_TELEMETRY_AT_REST", "false").lower() == "true"


def is_at_rest_enabled() -> bool:
    return _ENCRYPT_AT_REST


def store_telemetry_encrypted(link_id: str, point: dict) -> dict:
    """
    Wrap a telemetry point with an encrypted payload for at-rest storage.
    If encryption is disabled via ENCRYPT_TELEMETRY_AT_REST=false, returns
    the point unchanged so the DB writer can insert normally.
    """
    if not _ENCRYPT_AT_REST:
        return point
    import json
    encrypted = encrypt(json.dumps({
        "latency_ms": point.get("latency_ms"),
        "jitter_ms": point.get("jitter_ms"),
        "packet_loss_pct": point.get("packet_loss_pct"),
        "bandwidth_util_pct": point.get("bandwidth_util_pct"),
        "rtt_ms": point.get("rtt_ms"),
    }))
    # Keep numeric columns too — TimescaleDB aggregate queries need them.
    return {**point, "encrypted_payload": encrypted}


def read_telemetry_encrypted(row: dict) -> dict:
    """
    Read a telemetry row: if encrypted_payload is present and non-empty,
    decrypt it and use the plaintext as the canonical values, overriding
    the parallel numeric columns.
    """
    enc = row.get("encrypted_payload")
    if not enc:
        return row
    try:
        import json
        pt = json.loads(decrypt(enc))
        return {**row, **pt}
    except Exception:
        return row
