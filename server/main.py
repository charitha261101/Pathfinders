...
# -- Reset QoS/hosts in every conceivable exit path --
# Lifespan shutdown covers Ctrl+C and clean uvicorn exit, but closing the
# console window with the X button or `taskkill` skips it. atexit + signal
# handlers cover those cases so the host's internet is always restored.
def _emergency_qos_cleanup() -> None:
    try:
        from server.app_qos.priority_manager import _enforcer as _qos_enforcer
        _qos_enforcer.clear_all_rules()
    except Exception as exc:
        print(f"[main] emergency cleanup skipped: {exc}")


atexit.register(_emergency_qos_cleanup)


def _signal_cleanup(signum, frame):
    _emergency_qos_cleanup()
    # Re-raise as KeyboardInterrupt so uvicorn shuts down cleanly
    raise SystemExit(0)
...
for _sig in (signal.SIGINT, signal.SIGTERM):
    try:
        ...
        # signal() only works on the main thread on Windows
        pass
...
from server import redis_broker
...
async def lifespan(app: FastAPI):
    ...
    # Panic-cleanup any stale QoS state from a previous crashed process,
    # so the host's internet is guaranteed healthy at startup.
    try:
        from server.app_qos.priority_manager import _enforcer as _qos_enforcer
        _qos_enforcer.clear_all_rules()
    except Exception as exc:
        print(f"[main] startup QoS cleanup skipped: {exc}")
    # Connect the Redis pub/sub broker (CLAUDE.md §6.3).
    # No-op fallback if Redis is unavailable.
    await redis_broker.get_broker()
    ...
    sel_task = asyncio.create_task(selective_degrader.sweeper_loop())
    yield
    # Clean up traffic shaping policies on shutdown
    traffic_shaper.remove_all_policies()
    selective_degrader.stop_all()
    try:
        from server.app_qos.priority_manager import _enforcer as _qos_enforcer
        _qos_enforcer.clear_all_rules()
    except Exception as exc:
        print(f"[main] shutdown QoS cleanup skipped: {exc}")
    sim_task.cancel()
    pred_task.cancel()
    ibn_task.cancel()
    ws_task.cancel()
    sel_task.cancel()
    broker = await redis_broker.get_broker()
    await broker.close()
...
class HealthThresholdRequest(BaseModel):
    threshold: float
...
def _require_lstm_toggle_role(authorization: Optional[str] = __import__('fastapi').Header(None)):
    """
    JWT-claims role check for the LSTM toggle. Accepts BUSINESS_OWNER,
    SUPER_ADMIN, NETWORK_ADMIN, IT_MANAGER -- matches the existing
    app_priority router pattern so multi-tenant DB users (app_users table)
    work too, not just the in-memory seed users.
    """
    import jwt as _jwt
    _secret = os.getenv("JWT_SECRET", "pathwise-dev-secret-change-in-production")
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(401, "Not authenticated")
    try:
        claims = _jwt.decode(authorization.split(" ", 1)[1], _secret, algorithms=["HS256"])
    except Exception:
        raise HTTPException(401, "Invalid token")
    if claims.get("role") not in ("BUSINESS_OWNER", "SUPER_ADMIN", "NETWORK_ADMIN", "IT_MANAGER"):
        raise HTTPException(403, "Insufficient permissions")
    return claims


@app.post("/api/v1/admin/lstm-toggle", dependencies=[Depends(_require_lstm_toggle_role)])
...
@app.get("/api/v1/admin/health-threshold")
async def get_health_threshold():
    """Req-Func-Sw-4: read the admin-configurable steering trigger threshold."""
    return {
        "threshold": state.health_score_threshold,
        "alert_threshold": alerts.HEALTH_THRESHOLD,
        "suppression_window_s": alerts.SUPPRESSION_WINDOW,
    }


@app.post("/api/v1/admin/health-threshold", dependencies=[Depends(require_role("NETWORK_ADMIN"))])
async def set_health_threshold(req: HealthThresholdRequest):
    """Req-Func-Sw-4: update the steering trigger threshold at runtime."""
    if not (0 <= req.threshold <= 100):
        return {"error": "threshold must be between 0 and 100"}
    previous = state.health_score_threshold
    state.health_score_threshold = req.threshold
    alerts.update_config(threshold=req.threshold)
    audit.log_event(
        "SYSTEM", actor="admin",
        details=f"Health score threshold updated: {previous:.1f} -> {req.threshold:.1f}",
    )
    return {
        "threshold": state.health_score_threshold,
        "previous": previous,
...
@app.get("/api/v1/lstm/accuracy")
async def get_lstm_accuracy():
    """
    Req-Qual-Perf-1: expose most recent LSTM accuracy metrics.
    Populated by ml/scripts/evaluate.py which writes results to
    ml/checkpoints/accuracy.json on every training/evaluation run.
    """
    import json as _json
    acc_path = Path("ml/checkpoints/accuracy.json")
    if acc_path.exists():
        try:
            with open(acc_path) as f:
                data = _json.load(f)
            state.lstm_accuracy = data
            return data
        except Exception as e:
            return {"error": f"Failed to read accuracy file: {e}"}
    if state.lstm_accuracy:
        return state.lstm_accuracy
    return {
        "available": False,
        "message": "No accuracy data yet. Run: python ml/scripts/evaluate.py",
    }
...
async def sandbox_validate(req: SandboxValidationRequest):
    # Publish validation request on pathwise:validation:request
    broker = await redis_broker.get_broker()
    await broker.publish_validation_request({
    ...
    # Publish result on pathwise:validation:result
    await broker.publish_validation_result({
        "report_id": report.id,
        "result": report.result.value,
        "source_link": req.source_link,
        "target_link": req.target_link,
        "execution_time_ms": report.execution_time_ms,
        "within_sla_5s": report.execution_time_ms < 5000,
        "timestamp": time.time(),
    })
...
async def apply_routing_rule(req: ApplyRuleRequest):
    ...
    # Publish to pathwise:steering:trigger for downstream consumers
    broker = await redis_broker.get_broker()
    await broker.publish_steering({
        "rule_id": rule.id,
    ...
    return {
        ...
        "traffic_classes": rule.traffic_classes,
        "execution_time_ms": round(execution_ms, 2),
        "within_sla_50ms": execution_ms < 50.0,
...
async def scoreboard_broadcast_loop():
    ...
    while True:
        if _ws_clients or broker:
            ...
            # Also publish on pathwise:dashboard:updates so any other service
            # (future microservices, external consumers) receives the same feed.
            try:
                await broker.publish_dashboard(payload)
                # Per-link telemetry channel for downstream analytics consumers
                for link_id, data in payload.get("links", {}).items():
                    await broker.publish_telemetry(link_id, {
                        "link_id": link_id,
                        "health_score": data.get("health_score"),
                        "confidence": data.get("confidence"),
                        "latency": data.get("latency_current"),
                        "jitter": data.get("jitter_current"),
                        "packet_loss": data.get("packet_loss_current"),
