...
# Req-Qual-Sec-1: enforce TLS on the Postgres connection unless explicitly
# disabled for local dev. sslmode=require forces TLS; prefer doesn't fail
# if TLS is unavailable. Production deployments should also use sslmode=verify-full
# with DATABASE_SSLROOTCERT pointing at the Postgres server's CA cert.
_PG_SSLMODE = os.getenv("DATABASE_SSLMODE", "prefer")
...
def _try_pg():
    ...
    try:
        ...
        if _PG_SSLMODE and "sslmode=" not in _PG_URL:
